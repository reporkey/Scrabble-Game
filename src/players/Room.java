...
public class Room extends JFrame {
	...
	private static JLabel P2, P3, P4, P5, P6, P7, R7,R2,R3,R4,R5,R6;
	 ...
	 * Create the frame.
	 */
	public Room(String name, ArrayList<PlayerC> potentialPlayers, ArrayList<PlayerC> players, int randomNum,
			Socket client) {
	...
	public void initialize() {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ChessBoard game = new ChessBoard(players, client);
		...
		btnInvite.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (C1.isSelected()) {
						arr.put(potentialPlayers.get(0));
						R2.setText("WAIT");
					}
					if (C2.isSelected()) {
						arr.put(potentialPlayers.get(1));
						R3.setText("WAIT");
					}
					if (C3.isSelected()) {
						arr.put(potentialPlayers.get(2));
						R4.setText("WAIT");
					}
					if (C4.isSelected()) {
						arr.put(potentialPlayers.get(3));
						R5.setText("WAIT");
					}
					if (C5.isSelected()) {
						arr.put(potentialPlayers.get(4));
						R6.setText("WAIT");
					}
					if (C6.isSelected()) {
						arr.put(potentialPlayers.get(5));
						R7.setText("WAIT");
					}
					sendMsg.put("method", "request invite");
					sendMsg.put("players", arr);
					output.write(sendMsg.toString() + "\n");
					output.flush();
					// {"method":"request
					// invite","from":"3","players":[{"name":"1","port":"8080","score":"0","reday":"false","ip":"localhost/127.0.0.1",}]}
	...
	public static void setReady(ArrayList<PlayerC> players){
		for(PlayerC player:players)
		{
			if(player.getName().equals(P2.getText()))
				R2.setText("READY");
			if(player.getName().equals(P3.getText()))
				R3.setText("READY");
			if(player.getName().equals(P4.getText()))
				R4.setText("READY");
			if(player.getName().equals(P5.getText()))
				R5.setText("READY");
			if(player.getName().equals(P6.getText()))
				R6.setText("READY");
			if(player.getName().equals(P7.getText()))
				R7.setText("READY");
		}
	...
	public static int showMsg(String invitor, String name) {
		String[] buttons = { "Yes", "No" };
		int n = 0;
		if (!invitor.equals(name)) {
			String mgs = "A invite message from" + invitor;
			n = JOptionPane.showOptionDialog(null, "Do you want to play a game?", mgs, JOptionPane.DEFAULT_OPTION,
...
class messLis extends Thread {
	...
	private ArrayList<PlayerC> players;
	private Socket client;
	private String name;
	...
	public void run() {
		...
		try {
			...
			while ((msgStr = input.readLine()) != null) {
				...
				case "request invite": {
					// {"method":"request invite","from":"3"}
					String invitor = msg.getString("from");
					if (!invitor.equals(name)) {
						int choose = Room.showMsg(invitor, name);
						JSONObject invite = new JSONObject();
						invite.put("method", "response invite");
						invite.put("value", choose);
						output.write(invite.toString() + "\n");
						output.flush();
					}
					break;
				}
